...
def run_epoch(model, data):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        losses_all_batch.append(losses)
    all_losses = np.array(losses_all_batch)
    avg_loss_time = np.mean(all_losses, axis=0)
    with open('5_1_{}.txt'.format(args.model), 'w') as f:
        f.write('{}\n'.format(args.model))
        for ls in avg_loss_time:
            f.write('{}\n'.format(ls))
