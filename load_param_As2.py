...
parser.add_argument('--save_dir', type=str, default='full_exp/',
...
i = 0
# while not os.path.exists(experiment_path + "_" + str(i)):
#     i += 1
...
print("Loading model parameters from best_params.pt")

file_path=args.save_dir
file_path=file_path.replace("'","")
full_file_path=file_path.split("/")
file_path=full_file_path[-1]

if 'save_best' not in args.save_dir:
    file_path=file_path+"_save_best_"
else:
    file_path=file_path+"_"

file_path_ = file_path + str(i)
full_file_path[-1] = file_path_
file_path__ = "/".join(full_file_path)


file_path=file_path__+"/"

argsdict['save_dir'] = file_path


model.load_state_dict(torch.load(os.path.join(file_path, 'best_params.pt')))


print("Load model successfully from "+args.save_dir+'best_params.pt')

# python -i load_param.py --model=RNN --optimizer=ADAM --initial_lr=0.0001 --batch_size=20 --seq_len=50 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35
# python -i load_param.py --model=RNN --optimizer=ADAM --initial_lr=0.0001 --batch_size=20 --seq_len=50 --hidden_size=1500 --num_layers=2 --dp_keep_prob=0.35 --save_best
